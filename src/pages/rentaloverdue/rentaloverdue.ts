...
@IonicPage()
@Component({
  selector: 'page-rentaloverdue',
  templateUrl: 'rentaloverdue.html',
})

export class RentalOverduePage {
  ...
  returndetails() {
    ...
    this.authService.postData(data, 'rent/getRentalOverdueList').then((result) => {
      this.rentalOverdueList = result;

      if(Data[0] == 'Within 60 days'){
          this.rentdetailsall = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS <= 60 && x.CASE_NO == 0);
          this.rentdetails    = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS <= 60 && x.CASE_NO == 0);
      }else if(Data[0] == 'Above 60 to 180 days'){
          this.rentdetailsall = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS >= 61 && x.WITHIN_DAYS <= 180 && x.CASE_NO == 0);
          this.rentdetails    = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS >= 61 && x.WITHIN_DAYS <= 180 && x.CASE_NO == 0);
      }else if(Data[0] == 'Above 180 days'){
          this.rentdetailsall = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS >= 181 && x.CASE_NO == 0);
          this.rentdetails    = this.rentalOverdueList.filter(x=> x.WITHIN_DAYS >= 181 && x.CASE_NO == 0);
      }else if(Data[0] == 'Legal Case'){
          this.rentdetailsall = this.rentalOverdueList.filter(x=> x.CASE_NO > 0);
          this.rentdetails    = this.rentalOverdueList.filter(x=> x.CASE_NO > 0);
      }else{
