...
@IonicPage()
@Component({
  selector: 'page-returnchequeattachement',
  templateUrl: 'returnchequeattachement.html',
})
export class ReturnchequeattachementPage {
  ...
  CASE_ID=this.navParams.get('data');
...
insertAttachments(){
  ...
  uploadData.CASE_REQUEST_ID = this.CASE_ID[0].CASE_REQUEST_ID
  ...
  this.authService.postData(uploadData,'returncheque/ReturnchequeAttachmentsInsert').then((result) => {
    this.presentLoadingDefault(false);
    this.presentToast("Comments successfully saved");
    this.insertedValues = result;
    this.Getpaymentattachment(uploadData.CASE_REQUEST_ID);
  }, (err) => {
    this.presentLoadingDefault(false);
...
ionViewWillLoad() {
  ...
  let CASE_REQUEST_ID = this.CASE_ID[0].CASE_REQUEST_ID
  this.Getpaymentattachment(CASE_REQUEST_ID);
}
...
Getpaymentattachment(CASE_REQUEST_ID){
  this.presentLoadingDefault(true);
  this.authService.getData({},'returncheque/GetreturnchequattachList/'+CASE_REQUEST_ID+'').then((result) => {
