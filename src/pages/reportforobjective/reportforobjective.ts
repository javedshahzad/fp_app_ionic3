...
@IonicPage()
@Component({
  selector: 'page-reportforobjective',
  templateUrl: 'reportforobjective.html',
})
export class ReportForObjective {
  ...
  gettask() {
    ...
    this.authService.postData(params, 'task/getReportingUserListForAssignedObjective').then((result) => {
  ...
  getObjectiveList(user_id:any,reportType: any){
    ...
    if (reportType == 'Weekly')
      ...
      modelpage = 'ReportForAssignedObjectivePage';
  ...
  getItems(searchbar) {
    ...
    if (q.trim() == '') {
      this.taskdetails = this.taskdetailsearchall.dailytasklist;
      return;
    }

    this.taskdetails = this.taskdetailsearchall.dailytasklist.filter((v) => {
