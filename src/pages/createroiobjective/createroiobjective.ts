...
@IonicPage()
@Component({
  selector: 'page-createroiobjective',
  templateUrl: 'createroiobjective.html',
})

export class CreateRoiObjectivePage {
  today: any;
  ...
  inserttaskComments() {
    ...
    let commentsData = {
      created_by:  this.comments_type[0].reporting_user_id,
      modified_by: this.comments_type[0].reporting_user_id,
      insert_type: this.comments_type[0].label_type,
      COMMENTS: JSON.stringify(COMMENTS_txt),
      comments_type: this.type_string,
      login_user_id: this.user.UserInfoId,
      label_type: this.comments_type[0].label_type,
      start_date: this.comments_type[0].start_date,
      end_date: this.comments_type[0].end_date,
      selected_date: this.comments_type[0].selected_date
    } 
 
    if(COMMENTS_txt.length >0){
        this.presentLoadingDefault(true);
        this.authService.postData(commentsData, 'task/getInsertRoiObjectives').then((result) => {
            this.presentLoadingDefault(false);
            ...
            if(this.comments_type[0].reporting_user_id != this.user.UserInfoId){
             
                for(let i=0; i< COMMENTS_txt.length; i++){

                    let push_message = {} as any;
                    push_message.title = this.user.Surname;
                    push_message.content = 'ROI Objective';
                    push_message.message = COMMENTS_txt[i].name;
                    push_message.app_platform = app_platform;
                    push_message.user_info_id = this.comments_type[0].reporting_user_id;
                    push_message.loggedin_user_id = this.user.UserInfoId;
                    push_message.trans_type = this.comments_type[0].label_type;
