...
@IonicPage()
@Component({
  selector: 'page-contractcustomerdetails',
  templateUrl: 'contractcustomerdetails.html',
})
export class ContractCustomerDetailsPage {
  ...
  SearchrtcaseDetail() {
    ...
    if (case_val != '') {
      ...
      this.customerContract = filterData;
    } else {
      this.customerContract = this.searchdocumenttrackingdetails.filter(x=>  x.CONTRACT_SUPPLIER == 0);
    }
  ...
  filter(item) {
    ...
    let _lease_val = item['CUSTOMERNAME'] ? item['CUSTOMERNAME'].toString().toUpperCase() : '';
