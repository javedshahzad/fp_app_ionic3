...
export class TaskManagementPage {
  ...
  getuser() {
    ...
    this.authService.getData({}, 'task/UserList/').then((result: any) => {
      ...
      console.log(' Get User Seconds:', seconds);
  ...
  gettask() {
    ...
    this.authService.getData({}, 'task/TaskManagementSummaryv1/' + this.user.UserInfoId).then((result) => {
      ...
      if (this.taskdetails.dailytasklist.length > 0) {
        this.thirdslide = 1;
      }

      for (let j = 0; j < this.taskdetails.roiweeklypoints.length; j++) {
        var roicount = parseInt(this.taskdetails.roiweeklypoints[j].POINT_NO);
        this.roiPointsCount += roicount;
      }

      
      let time_af = new Date();
      let seconds = Math.round((time_af.getTime() - time_bf.getTime()) / 1000);
      console.log(' Task Seconds:', seconds);
  ...
  getWeeklyCommentsCount() {
    ...
    this.authService.postData(data, 'task/weeklycommentsCount').then((result) => {
      ...
      let time_af = new Date();
      let seconds = Math.round((time_af.getTime() - time_bf.getTime()) / 1000);
      console.log(' Weekly Count:', seconds);
  ...
  openTaskdailycmd(USER_INFO_ID: any, USER_SURNAME: any) {
    ...
    this.authService.postData(data, 'task/weeklycomments_user_wise_list').then((result: any) => {
      ...
      this.myModalData = [result, USER_INFO_ID, this.taskdetails.dailytasklist.length, USER_SURNAME];
  ...
  getReportingUserList(org: any) {
    ...
    let modelpage = '';
    modelpage = 'RoiReportingUserList';
