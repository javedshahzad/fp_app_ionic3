...
@IonicPage()
@Component({
  selector: 'page-utilityattachment',
  templateUrl: 'utilityattachment.html',
})
export class UtilityAttachmentPage {
  ...
  insertAttachments() {
    ...
    let uploadData = this.paymentattachmentForm.value;    
      uploadData.GROUPID = Data[0].groupid
      ...
      this.authService.postData(uploadData, 'utility/getUtilityAttachmentsInsert').then((result) => {
  ...
  Getpaymentattachment() {
    ...
    this.presentLoadingDefault(true);
    this.authService.postData(Data[0], 'utility/getUtilityPaymentAttachment').then((result) => {
