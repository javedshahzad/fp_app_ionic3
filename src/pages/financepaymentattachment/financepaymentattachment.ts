...
@IonicPage()
@Component({
  selector: 'page-financepaymentattachment',
  templateUrl: 'financepaymentattachment.html',
})
export class FinancePaymentAttachmentPage {
  ...
  getCommonAttchmentList(){

     let params = {} as any;;
         params.user_info_id = this.user.UserInfoId;

    //this.presentLoadingDefault(true);
    this.authService.postData(params, 'payment/getCommomAttachmentTypeListAll').then((result) => {
      this.commonattachmentfilelist = result;
      //this.presentLoadingDefault(false);
    }, (err) => {
      //this.presentLoadingDefault(false);
      this.presentToast(err);
