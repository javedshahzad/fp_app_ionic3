...
@IonicPage()
@Component({
  selector: 'page-rentaloverduecomment',
  templateUrl: 'rentaloverduecomment.html',
})
export class RentalOverdueCommentPage {
  ...
  insertrentComments() {
    ...
    this.authService.postData(rentcommentsData, 'rent/getInsertRentalOverdueComment').then((result) => {
  ...
  ionViewWillLoad() {
    ...
    this.presentLoadingDefault(true);
    this.authService.postData(push_message, 'rent/getRentalOverdueComments').then((result) => {
      this.rentcommentsdetails = result;
      this.presentLoadingDefault(false);
      if (this.rentcommentsdetails.length > 0) {
        this.presentLoadingDefault(false);
...
uploadAudioNewComment() {
  ...
  let rentcommentsData = this.rentcommentsForm.value;
    rentcommentsData.modified_by = this.user.UserInfoId;
    rentcommentsData.modified_by = this.user.UserInfoId;
    rentcommentsData.ReferenceType = 'RentProp COMMENT';
