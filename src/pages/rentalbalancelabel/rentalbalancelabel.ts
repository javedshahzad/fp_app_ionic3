...
@Component({
  selector: 'page-rentalbalancelabel',
  templateUrl: 'rentalbalancelabel.html',
})
export class RentalBalanceLabelPage {
  ...
  returndetails() {
    ...
    this.authService.postData(data, 'rent/getRentalBalanceLabelWise').then((result) => {
      ...
      this.above_60_180days = this.rentdetailsall[0].WITHIN_60_180DAYS;
  ...
  openDetailModal(type: any) {
    ...
    if(showmodal == 1){
      ...
      let myModal: Modal = this.modal.create('RentalBalancePage', { data: myModalData }, myModalOptions);
