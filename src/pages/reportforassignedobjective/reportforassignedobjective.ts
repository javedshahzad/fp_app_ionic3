...
@IonicPage()
@Component({
  selector: 'page-reportforassignedobjective',
  templateUrl: 'reportforassignedobjective.html',
})
export class ReportForAssignedObjectivePage {
  ...
  constructor(public platform: Platform, public navCtrl: NavController, public navParams: NavParams,
    ...
    this.ASSIGNED_USER_INFO_ID = this.modaltype[0].user_info_id
  ...
  generateReport(){    
    ...
    this.authService.postData(TASK_DATA, 'task/getReportManagerAssignedObjective').then((result:any) => {
  ...
  movetoobjective(i: number, COMMENTS_CHILD_ID: any): void {
    console.log(COMMENTS_CHILD_ID);
    let params = {
      user_info_id: this.user.UserInfoId,
  ...
  removeCommentsFromReport(i: number, COMMENTS_CHILD_ID: any){
    console.log(COMMENTS_CHILD_ID);
    let params = {
      user_info_id: this.user.UserInfoId,
      comments_child_id: COMMENTS_CHILD_ID 
    ...
    this.presentLoadingDefault(true);
    this.authService.postData(params, 'task/getremoveResultFrmAssignedObjective').then((result) => {
      console.log('result',result);
      this.presentLoadingDefault(false);  
      this.ionViewDidLoad();
    }, (err) => {
