...
@IonicPage()
@Component({
  selector: 'page-lpoceomultipleapproval',
  templateUrl: 'lpoceomultipleapproval.html',
})

export class lpoCeoMultipleApprovaloption {
  ...
  getLpoData(uType) {
    ...
    this.authService.postData(context, 'Lpo/GetNewLpoList').then((result) => {
      ...
      if (this.tab_name == 'uganda') {
        this.Lpomanament_data = this.Lpomanament_dataAll.filter(x => x.OPERATIONAL_UNIT_NAME == 'UG');
        ...
        this.Lpomanament_data = this.Lpomanament_dataAll.filter(x => x.OPERATIONAL_UNIT_NAME == 'TREPPAN');
      } else {
