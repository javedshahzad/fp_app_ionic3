...
@IonicPage()
@Component({
  selector: 'page-paymentattachment',
  templateUrl: 'paymentattachment.html',
})
export class PaymentAttachmentPage {
  ...
  insertAttachments(){
    ...
    uploadData.PAYMENT_REQUEST_ID = this.payment_req_id[0].PAYMENT_REQUEST_ID
    ...
    this.authService.postData(uploadData,'payment/PaymentAttachmentsInsert').then((result) => {
      this.presentLoadingDefault(false);
      this.presentToast("Comments successfully saved");
      this.insertedValues = result;
      this.Getpaymentattachment(uploadData.PAYMENT_REQUEST_ID);
    }, (err) => {
      this.presentLoadingDefault(false);
  ...
  ionViewWillLoad() {
    ...
    let PAYMENT_REQUEST_ID = this.payment_req_id[0].PAYMENT_REQUEST_ID
    this.Getpaymentattachment(PAYMENT_REQUEST_ID);
  }
  resetForm(){
 ...
 Getpaymentattachment(PAYMENT_REQUEST_ID){
    this.presentLoadingDefault(true);
    this.authService.getData({},'payment/GetPaymentattachList/'+PAYMENT_REQUEST_ID+'').then((result) => {
