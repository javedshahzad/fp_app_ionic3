...
export class DailytaskcommentPage {
  ...
  ionViewDidLoad() {
    ...
    if (this.comments_type[0] == "Next" || this.comments_type[0] == "Current") {
      this.label_title = this.comments_type[0] + ' ' + 'Week ROI';
      this.show_comment_list = -1;
      this.gettypecount(this.comments_type[0]);
      this.getInsertRoiHeader(this.comments_type[0]);
    }
    else if (this.comments_type[0] == 'ROI NOTIFICATION') {
      this.label_title = 'Current Week ROI';
      this.show_comment_list = -1;
      this.gettypecount('Current');
      this.getInsertRoiHeader('Current');

    } else {
      this.show_comment_list = 0;
      this.label_title = 'Last Week ROI';
  ...
  editcomments1(item) {
    ...
    this.taskcommentseditForm.controls.comments.reset();
    ...
    if (this.editcomment.length > 0) {
      ...
      for (var i = 0; i < this.editcomment.length; i++) {
        ...
        //this.editcomment[i].COMMENTS = this.createTextLinks(this.editcomment[i].COMMENTS);
        ...
        control.push(this.formBuilder.group({
          ...
          manager_id: [manager_id, Validators.required],
          no_of_week: [this.editcomment[i].NO_OF_WEEK, Validators.required],
          is_mom: [this.editcomment[i].IS_MOM, Validators.required],
          mom_id: [this.editcomment[i].MOM_ID, Validators.required],
          audio_count: [this.editcomment[i].AUDIO_COUNT, Validators.required]
        }));
      }
  ...
  moveToResult(i: number, data: any): void {
    ...
    this.authService.postData(params, 'task/getMoveObjectiveToResult').then((result) => {
      ...
      this.authService.postData(data, 'task/dailycommentscount').then((result) => {
        this.taskcommentscount = result;
        this.openModal(this.type_string);
      }, (err) => {
        this.presentLoadingDefault(false);
        this.presentToast(err);
      });
  ...
  getInsertRoiHeader(comments_type: any) {
    let time_bf = new Date();
    let commentsData = {
      created_by: this.user.UserInfoId,
      modified_by: this.user.UserInfoId,
      insert_type: comments_type,
      comments_type: 'R'
    }
    ...
    this.authService.postData(commentsData, 'task/getInsertRoiWeekLabel').then((result) => {
      ...
      let time_af = new Date();
      let seconds = Math.round((time_af.getTime() - time_bf.getTime()) / 1000);
      console.log(' Insert ROI Header Seconds:', seconds);
    }, (err) => {
      this.presentToast(err);
    });
  }

  openmompage(index: any, mom_id: any) {
  ...
  uploadAudio(i: number, data: any) {
    ...
    let myModalData = [{
      user_info_id: this.user.UserInfoId,
      comments_id: data.value.COMMENTS_ID,
      comments_child_id: data.value.child_id
    }]
  ...
  uploadAudioForNewComment() {

    const myModalOptions: ModalOptions = {
      enableBackdropDismiss: false
    };

    let COMMENTS_txt = [{ name: 'Recording', manager_id: null }];
    COMMENTS_txt = COMMENTS_txt.filter((x) => x.name != 'null' && x.name != null && x.name.trim() != '');
    let commentsData = {
      created_by: this.user.UserInfoId,
      modified_by: this.user.UserInfoId,
    ...
    if (COMMENTS_txt.length > 0) {
      ...
      this.authService.postData(commentsData, 'task/getRoiInsertWhileAudioRecording').then((result) => {
        this.presentLoadingDefault(false);
        console.log(result);
        ...
        console.log(ResultData[0].comments_id, ResultData[0].comments_id);

        let myModalData = [{
          user_info_id: this.user.UserInfoId,
          comments_id: ResultData[0].comments_id,
          comments_child_id: ResultData[0].comments_child_id
        }]
